#!/bin/bash
##############################################################################
#  ddg_saturation_job_prod_1backrubSmoketest.sh      –  array-safe replica runner for Wynton SGE
#
#  Usage (from submit_all_saturation_prod_1backrubSmoketest.sh):
#     qsub ddg_saturation_job_prod_1backrubSmoketest.sh <pdb> <mutfile> <run_id> <project> [reslist]
#
...
##############################################################################
if (( $# < 4 )); then
  echo "Usage: qsub ddg_saturation_job_prod_1backrubSmoketest.sh <pdb> <mutfile> <run_id> <project> [residues.txt]"
  exit 1
fi
...
rosetta_ddg_run \
    -p  "${PDB_FILE}" \
    -cr /wynton/home/craik/kjander/ddg/RosettaDDGPrediction/RosettaDDGPrediction/config_run/ddg_kja_prod_1backrub.yaml \
